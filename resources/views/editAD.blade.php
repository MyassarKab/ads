...
<div class="submit-ad main-grid-border">
  <div class="container">
    <h2 class="head">Edit your advertising</h2>




    <div class="post-ad-form">
      <form   method="post" action="/Update-Advertising/{{$product->id}}"  enctype="multipart/form-data">
        ...
        <select class="Category" name="category_id" required>
          ...
          <?php foreach ($Categories as $key => $value): ?>
              <option value="{{$value->id}}" <?php if ($product->category_id==$value->id): ?>
								selected
              <?php endif; ?>>{{$value->name_en}}</option>
          <?php endforeach; ?>
        ...
        <div class="clearfix"></div>
        <label> Title In English<span>*</span></label>
        <input type="text" class="name" name="name" value="{{$product->name_en}}" placeholder="">
        <div class="clearfix"></div>
        <label> Description In English<span>*</span></label>
        <textarea class="mess" placeholder="Write few lines about your product" name="description">
					...
					<div class="clearfix"></div>
					<label> Title In Arabic<span>*</span></label>
					<input type="text" class="name" name="name" value="{{$product->name_ar}}" placeholder="">
					<div class="clearfix"></div>
					<label> Description In Arabic<span>*</span></label>
					<textarea class="mess" placeholder="Write few lines about your product" name="description">
					 ...
					 <div class="clearfix"></div>
					 <label> Title In Turkish<span>*</span></label>
					 <input type="text" class="name" name="name" value="{{$product->name_tr}}" placeholder="">
					 <div class="clearfix"></div>
					 <label> Description In Turkish<span>*</span></label>
					 <textarea class="mess" placeholder="Write few lines about your product" name="description">
        ...
        <input type="text" class="name" name="adress" value="{{$product->adress}}" placeholder="Your Adress">
        <div class="clearfix"></div>
        <label> Price <span>*</span></label>
        <input type="text" class="name" name="price" value="{{$product->price}}" placeholder="price">
        <div class="clearfix"></div>


				<label>Select Old Deal </label>
				<select class="Category" name="category_id" required>
					<option value="0"   disabled="disabled" >Select Deal </option>
					<?php foreach ($Deal as $key => $value): ?>
							<option value="{{$value->id}}"  > <?php if($value->urgently_kind==0){echo "Normal";}elseif ($value->urgently_kind==1) {
								echo "Featured";
							}else{
								echo "Urgent";
							} ?>
							...
							<option value="-1">New Deal</option>
					<?php endforeach; ?>
          ...
          <div class="row" style="    margin-bottom: 15px;">
                ...
                <button type="button" name="button" class="cardButton btn-radio <?php if ($product->urgently_kind==0): ?>	active  <?php endif; ?> ">
               ...
               <input type="checkbox" id="left-item" class="hidden" name="urgently_kind" value="0"  <?php if ($product->urgently_kind==0): ?>checked="true" <?php endif; ?>>
     				</div>
            <!--  -->
      ...
      <div class="clearfix"></div>
        ...
        <input type="date" class="date" id="expiry_date" value="{{$product->expiry_date}}" name="expiry_date" placeholder="" onchange="check()">
